...
export function copy(season: Season = currentSeason()) {
  ...
  return {
    ...
    campaign: {
      ...
      bar: `Lanceringstilbud: ekstra eksemplar med i pakken – værdi ${formatDkk(EXTRA_PRINT_DKK[format])} · til og med ${kampagneDato}`,
      title: `Lanceringstilbud – værdi ${formatDkk(EXTRA_PRINT_DKK[format])}`,
      body: `Bestil til og med ${kampagneDato}, og få ét ekstra eksemplar af dit billede med i pakken. Samme størrelse, samme ramme – til den, der også husker det.`,
