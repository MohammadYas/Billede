...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  // one preview, a few addresses: after that the form says "sent" and sends nothing, so a preview can
  // never be turned into a machine for mailing strangers from our domain
  const sent = Number(meta.share_sent_count ?? 0);
  if (sent >= 5) return NextResponse.json({ ok: true });
