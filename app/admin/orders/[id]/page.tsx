    ...
    <main className="wrap admin" style={{ paddingTop: 'var(--s3)', paddingBottom: 'var(--s9)' }}>
      <div className="container" style={{ display: 'grid', gap: 'var(--s6)' }}>
        ...
        <section className="small adm-facts">
          <p><strong>Kunde:</strong> {order.customer_name ?? '—'} · {order.customer_email ?? '—'} · {order.customer_phone ? <a href={`tel:${order.customer_phone.replace(/s/g, '')}`}>{order.customer_phone}</a> : '—'}{order.customer_email && <> · <a href={`/admin/beskeder?til=${encodeURIComponent(order.customer_email)}`}>Skriv til kunden</a> · <a href={`/admin/beskeder/${encodeURIComponent(order.customer_email.toLowerCase())}`}>Samtale</a></>}</p>
          ...
          {repeatLink(order)
            ? <p><strong>Gentagelseslink (kunden har det i kvitteringen):</strong> <span className="muted">{repeatLink(order)}</span></p>
            : <p className="small muted">Gentagelseslink laves først, når ordren er betalt — det er kvitteringen, kunden får det i.</p>}
        </section>
