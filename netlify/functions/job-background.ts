 ...
 * Invoked by lib/jobs.ts `enqueue()` with the shared JOB_SECRET. Restoration ≈ 40 s, colour ≈ 30 s,
 * print final ≈ 2 min — none of which fits a synchronous function (10–26 s) or a streamed one (60 s).
 */
export default async (req: Request) => {
  ...
  if (!body.kind || !body.orderId || !/^[0-9a-f-]{36}$/.test(body.orderId)) return new Response('bad request', { status: 400 });
