...
No UI kit. Three self-hosted fonts. One third-party script (Meta Pixel, after consent).
...
Inbox: mail to hej@ (Resend receiving → `/api/webhooks/resend`) and the contact form (`/kontakt` → `/api/contact`) land in `messages`; admin → Beskeder answers from hej@. See HANDOFF.md.
