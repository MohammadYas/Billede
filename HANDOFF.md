...
- **Mail is live (2026-09-08 12:10 UTC):** billedearv.dk is Verified in Resend (DKIM, SPF on , receiving MX on ), the key in the env is Full access, the webhook  → /api/webhooks/resend exists with its secret in Netlify. Verified end to end: three customer mails delivered to the owner's gmail; a mail to hej@ was received by Resend, signed, fetched and filed in admin → Beskeder. Lesson: the domain had first been added to Resend as "biiledearv.dk" — always read the API's domain list, not the dashboard's spelling.
- **Owner still has to (in this order):**
  1. Stripe Dashboard: webhook URL →  (same signing secret), statement descriptor BILLEDEARV.DK, business website/name.
  2. Street address in ; lawyer reads /handelsbetingelser and /privatliv →  in Netlify.
  3. Meta pixel id + CAPI token in Netlify when ads start;  on every ad link (admin → Kilder reads them).
  4. Delete the two test orders in admin ( = owner's phone test,  = Playwright).
