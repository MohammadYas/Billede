...
- **Ads, round 2:** `scripts/ads-concepts.mjs` (replaces ads-hero.mjs) renders six buying motives A–F with three layouts; every image carries "I ramme fra 599 kr. inkl. fragt" and "Se mit billede"; no slider knob in a static image; a split only on the same face. `docs/meta-ads-prompt.md` Fase 3 = the six ads with primary texts. Files in `work/ads/final/<koncept>-…` (git-ignored, back up).
