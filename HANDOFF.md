...
- **Meta ads are written, not launched:** `docs/meta-ads-prompt.md` is the prompt for Claude in Chrome (Fase 0 fundament → kampagne → annoncesæt → fire annoncer, alt pauset, stop før Udgiv), and `node scripts/ads-creatives.mjs` renders the before/after creatives (1:1 and 4:5) into `work/ads/creatives/` (git-ignored, rerun when the examples change).
