...
- **Mail is live (2026-09-08 12:10 UTC):** billedearv.dk is Verified in Resend (DKIM, SPF on `send`, receiving MX on `@`), the key in the env is Full access, the webhook `email.received` → /api/webhooks/resend exists with its secret in Netlify. Verified end to end: three customer mails delivered to the owner's gmail; a mail to hej@ was received by Resend, signed, fetched and filed in admin → Beskeder. The owner can also start a conversation from Beskeder ("Ny besked", or "Skriv til kunden" on an order). Lesson: the domain had first been added to Resend as "biiledearv.dk" — always read the API's domain list, not the dashboard's spelling.
- **Stripe is done (2026-09-08):** webhook at https://billedearv.dk/api/webhooks/stripe (checkout.session.*), descriptor BILLEDEARV.DK, prefix BILLEDEARV, website billedearv.dk — read back from the API.
- **Owner still has to (in this order):**
  ...
  3. Meta pixel id + CAPI token in Netlify when ads start; `?utm_source=…&utm_campaign=…&utm_content=…` on every ad link (admin → Kilder reads them).
  4. Delete the two test orders in admin (`ecf3aaeb` = owner's phone test, `6d8f65fc` = Playwright).
  5. Google Search Console: property billedearv.dk (the Google CNAME is already in DNS) → submit https://billedearv.dk/sitemap.xml.
